...
vk.updates.on('message_new', async (context: any, next: any) => {
	...
	if (!user_check) {
		...
		answer_check = false
		while (answer_check == false) {
			const answer3 = await context.question(`🧷 В потоке мыслей всплывает третий вопрос: \n\n ⌛ Раз в столетие на трепетливых кустиках распускаются цветы, которые подстраивают свой аромат таким образом, чтобы завлечь неосторожных. Если бы кустарник заманил вас, он имел бы запах... \n\n ❄ Свежести \n 🌩 Грозы \n 🌺 Цветов \n 🌱 Травы`,
				{
					keyboard: Keyboard.builder()
					.textButton({ label: '❄', payload: { command: 'coga' }, color: 'secondary' })
					.textButton({ label: '🌩', payload: { command: 'sliz' }, color: 'secondary' })
		...
		const data_answer: any = {
			...
			Подай заявку в гостиную: https://vk.com/club203252392
			...
			А также старосту факультета, именно к нему можно обращаться по всем вопросам — ${get_headman?.coga}
			
			Поменять факультет можно только на 2 курсе обучения.`,
			'puff': `ПУФФЕНДУЙ🎉🎊💛💛 — немного подумав, шляпа огласила вердикт для вас,
			ученик(ца) ${datas[0].name}!

			Подай заявку в гостиную: https://vk.com/club200655488
			...
			А также старосту факультета, именно к ней можно обращаться по всем вопросам — ${get_headman?.puff}
			
			Поменять факультет можно только на 2 курсе обучения.`,
			'sliz': `СЛИЗЕРИН🎉🎊💚💚 — немного подумав, шляпа огласила вердикт для вас,
			ученик(ца) ${datas[0].name}!

			Подай заявку в гостиную: https://vk.com/slytherin_hogonline
			...
			А также старосту факультета, именно к нему можно обращаться по всем вопросам — ${get_headman?.sliz}	
			...
			А также старосту факультета, именно к ней можно обращаться по всем вопросам — ${get_headman?.grif}
